	...
	class Page 
	{
		...
		*/
		public function __construct($total,$listRows=10,$query="",$ord=true)
		{
			...
			$this->listRows = $listRows;	//每页要显示的记录数

			$this->uri = $this->getUri($query);
			//返回不小于 value 的下一个整数,ceil() 返回的类型仍然是 float。
			...
			if (!empty($_GET["page"])) {
				...
				if($ord)
					//$ord为true则从第一页开始，否则为最后一页
					$page = 1;
				else
					$page = $this->pageNum; //$this->pageNum为总页数，最后一页
			}
			if ($total>0) {
				//匹配数字0-9
		...
		/*可以使用在对象外部直接获取私有成员属性limit和page的值 */
